        ...
        if not verify_jwt(token):
            ...
            if content:
                try:
                    ...
                    db.engine.execute("SELECT * FROM customer")

                    search_term = content["search"]
                    # ruleid:formatted-sql-query
                    inline = db.engine.execute(
                        "SELECT * FROM cutsomer WHERE username = '%s'" % search_term
                    )
                    str_query = "SELECT first_name, last_name, username FROM customer WHERE username = '%s';".format(
                        search_term
                    )
                    ...
                    return (
                        render_template_string(
                            template, dir=dir, help=help, locals=locals
                        ),
                        404,
                    )
